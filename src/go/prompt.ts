...
The user message gives you the board as ASCII (X is Black, O is White, . is empty), then facts the rules computed: the score if the game ended now, how many empty points are each side's territory or neutral, the captures and komi, every group on the board with its liberties, and the moves played recently. LEGAL MOVES is a numbered table of the moves you may play right now. Each row states what the rules prove about that stone: line (counted from the nearest edge), captures (enemy stones it lifts), libertiesAfter (your group's liberties once it lands), rescues (own stones it saves from atari), atari (enemy stones it leaves with one liberty), touchesOwn and touchesEnemy (neighbouring stones). The pass is listed only when the rules allow it to make sense, and its row says what it does. Pick exactly one entry by its number. Never invent a move that is not in the table.
