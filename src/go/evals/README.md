# Go: a model against the rules bot or a second model

`vs-bot.ts` plays one text model against `goHeuristic`, or against a second text model named by `--opponent`, through the real `/api/generate-action` endpoint, so the offered moves, the prompt and the reply protocol are the production ones. It is opt-in and live: every move spends model usage.
