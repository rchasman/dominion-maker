...
export function ReasoningPane({ votingData, modelStatuses }: ReasoningPaneProps) {
          ...
          {votingData.allResults.map((result, idx) => {
            ...
            return (
              <div key={idx} style={{
                ...
                borderBottom: idx < votingData.allResults.length - 1 ? "1px solid var(--color-border)" : "none"
              }}>
                <div style={{
                  fontSize: "0.75rem",
                  fontWeight: 600,
  ...
  if (!modelStatuses) {
    return (
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", padding: "var(--space-5) var(--space-4) var(--space-3)" }}>
        <div style={{ color: "var(--color-text-secondary)", fontSize: "0.7rem" }}>No reasoning data yet...</div>
      </div>
    );
  }
